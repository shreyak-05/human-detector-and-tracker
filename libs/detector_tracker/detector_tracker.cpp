...
DetectorTracker::DetectorTracker(
    ...
    std::shared_ptr<ITransformer> transformer, float confidence_threshold)
...
std::vector<Detection3D> DetectorTracker::get_3d_positions(
    const cv::Mat& frame) {
...
std::vector<Detection> DetectorTracker::post_process(const cv::Mat& output,
  ...
  for (int i = 0; i < reshaped_output.cols; ++i) {
    // YOLOv8 format: [cx, cy, w, h, class0_prob, class1_prob, ...,
    // class79_prob]
    ...
    float person_class_prob =
        reshaped_output.at<float>(4, i);  // class 0 = person
    ...
    // Final confidence = class_probability (no separate objectness in modern
    // YOLOv8)
  ...
  std::transform(indices.begin(), indices.end(), std::back_inserter(detections),
                 [&boxes, &confidences](int idx) {
                   return Detection{boxes[idx], confidences[idx], 0};
                 });
