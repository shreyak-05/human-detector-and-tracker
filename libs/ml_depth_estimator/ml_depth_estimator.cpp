...
/**
 * @file ml_depth_estimator.cpp
 * @brief Implementation of monocular depth estimation using ONNX models
...
Mat MLDepthEstimator::infer(const Mat& bgr) {
  ...
  Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
      memory_info_, input_data.data(), input_data.size(), input_shape.data(),
      input_shape.size());
...
float MLDepthEstimator::get_depth(const Mat& frame, Rect bbox) {
  ...
  int center_x = std::max(
      0, std::min(static_cast<int>(center.x), cached_depth_map_.cols - 1));
  int center_y = std::max(
      0, std::min(static_cast<int>(center.y), cached_depth_map_.rows - 1));
...
Mat MLDepthEstimator::get_cached_depth_map() const { return cached_depth_map_; }
