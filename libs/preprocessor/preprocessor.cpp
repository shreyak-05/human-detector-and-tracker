...
Mat Preprocessor::process(const Mat& frame) {
  // Create a blob from the image. This function will resize, pad
  // (scale=1.0/255), swap RB channels, and not crop.
  Mat blob =
      cv::dnn::blobFromImage(frame, 1.0 / 255.0, cv::Size(input_w_, input_h_),
