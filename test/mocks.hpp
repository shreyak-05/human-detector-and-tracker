 ...
 *
 * This mock class implements the IPreprocessor interface using Google Mock
 * (GMock). It allows setting expectations on method calls and controlling
 * return values during unit tests, enabling isolated testing of components that
 * depend on IPreprocessor without requiring real image processing.
 *
...
class MockPreprocessor : public IPreprocessor {
 public:
   ...
   *
   * @param frame Input image frame (can be matched with ::testing::_ for any
   * input)
 ...
 * It allows setting expectations on neural network forward pass calls and
 * controlling return values during unit tests, enabling isolated testing of
 * components that depend on INetwork without requiring real model loading and
 * inference.
 *
...
class MockNetwork : public INetwork {
 public:
   ...
   * and controlling return values in unit tests.
   *
   * @param blob Input blob tensor [N,C,H,W] (can be matched with ::testing::_
   * for any input)
   * @return Mocked return value (set via EXPECT_CALL)
   */
...
class MockDepthEstimator : public IDepthEstimator {
 public:
  MOCK_METHOD(float, get_depth, (const cv::Mat& frame, cv::Rect bbox),
              (override));
};
 ...
 * It allows setting expectations on coordinate transformation calls during unit
 * tests.
 */
class MockTransformer : public ITransformer {
 public:
  MOCK_METHOD(cv::Point3f, project_to_3d, (cv::Point2f pixel, float depth),
              (override));
};
